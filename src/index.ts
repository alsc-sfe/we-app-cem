...
import { InstantObj, OnlyData, DataCallback, EventCallback, EvCallback } from './types';
...
class CustomEventManager {
  ...
  trackShareData(dataPath: string | DataCallback, cb: DataCallback, instantObj: InstantObj = { instant: true }) {
    let callback = cb;
  ...
  trackShareDataOnce(dataPath: string | DataCallback, cb: DataCallback) {
  ...
  shareData(data: any) {
    ...
    try {
      ...
      this.trigger(SHARE_DATA_EVENT, currentData);
    } catch (error) {
      console.log(error);
    }
  }
